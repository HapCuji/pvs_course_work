...
+++++++++++++++
Следует сказать, что работа потоков имеет смысл, когда нужен общий доступ к одному адресному пространству!
Например, если требуется работа с одним массивом данных структурой или файлом.
В случае, если нужно выполнение двух не связных задач то лучше использовать новый процесс.
=> 
Для программы клиента (в месседжере) следует использовать потоки, 
т.к. контролировать ввод с клавиатуры и выводить ответ от сервера нужно в одном месте (один файл == один терминал  == одно окно). 
И переключать контекст между процессами будет выглядеть как задержка (видимо так оно и есть).
для сервера же будет выгоднее вести логирование в отдельном процессе (если это один логгер).
Если логер нужен для каждого процесса отдельный то лучше создавать несколько файлов (логов) 
и для каждого процесса свой логгер (~при работе с несколькими сокетами?).
*/
